...
class ThingController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($things->count() === 0) {
            return response()->json(['message'=> 'no things yet'], 404);
        }
    ...
    public function insert(Request $request)
    {
        ...
        if ($request->hasFile('photo') && $request->file('photo')->isValid()) {
            ...
            $request->file('photo')
                ->move(storage_path('app/images_things'), $filename);
        }
        $thing = Thing::create(
            [
    ...
    public function get(int $id)
    {
        ...
        if ($thing===null) {
            return response()->json(['status'=>'bad'], 404);
        }
    ...
    public function update(int $id, Request $request)
    {
        ...
        if (Auth::user()->id !== $thing->user_id) {
            return response()->json(['message' => 'not an owner of the thing'], 401);
        }
        ...
        if ($request->hasFile('photo') && $request->file('photo')->isValid()) {
            ...
            if ($thing->filename !== null) {
                unlink(storage_path('app/images_things').$thing->filename);
            }
            $ext = $request->file('photo')->guessExtension();
            $filename = md5(time()).'.'.$ext;
            $request->file('photo')
