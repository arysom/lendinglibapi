...
class ReservationController extends Controller
{
     ...
     * @return response
     */
    public function insert(int $thing_id, Request $request)
    {
        $start = Carbon::parse($request->input('start'));
        $end = Carbon::parse($request->input('end'));
        if ($end < $start) {
            return response('End earlier than start', 409);
        }
        ...
        if ($thing->reservations->count() > 0) {
            foreach ($thing->reservations as $value) {
                ...
                if ($start >= $value->start && $start <= $value->end) {
                    return response('Start overlapping', 409);
                }
                //if end inside
                if ($end >= $value->start && $end <= $value->end) {
                    return response('End overlapping', 409);
                }
            }
        }
        ...
        return response()->json(
            ['status'=>'ok', 'reservation' => $reservation], 201
        );
    }
    ...
    public function update(int $id, Request $request)
    {
        ...
        //can cancel
        if (Auth::user()->id === $reservation->user_id && $status !== 'canceled') {
        ...
        if (Auth::user()->id === $reservation->thing->id &&
